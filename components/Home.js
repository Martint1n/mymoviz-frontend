...
function Home() {
  ...
  // Movies list --- change array with fetch API
  ...
  const movies = newMovies.map((data, i) => {
    const overviewLimitedChar = [];
    if (data.overview.length > 250){
      overviewLimitedChar = [...data.overview.slice(0, 249), ' ...']
    }else {
      overviewLimitedChar = data.overview
    }
    const isLiked = likedMovies.some(movie => movie === data.original_title);
    return <Movie key={i} title={data.original_title} poster={`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} overview={overviewLimitedChar} updateLikedMovies={updateLikedMovies} isLiked={isLiked} />
